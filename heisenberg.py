...
"""
The below code was generated by Quinn Gleason
"""
# Pauli matrices
...
def simulate_heisenberg(size, temperature, num_sweeps, thermalization_sweeps, J=1.0):
    ...
    Returns:
        ...
        list of energy per spin measured after thermalization
    """
...
def sz_grid(lattice):
    """
    Return an LxL float array with s_z(i,j) = 0.5 * Tr(sigma_z @ S_z(i,j))
    For plus/minus sigma_z this is plus/minus 1
    """
    L = lattice.shape[0]
    grid = np.empty((L, L), dtype=float)
    for i in range(L):
        for j in range(L):
            Sz = lattice[i, j, 2]
            grid[i, j] = 0.5 * np.trace(sigma_z @ Sz).real
    return grid
